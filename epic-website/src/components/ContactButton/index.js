...
const ContactButton = (props) => {
  ...
  return (
    <div {...props.contactMorph} class="fixed bottom-0 right-0 -mr-8 bg-transparent z-40 overflow-hidden" onClick={() => props.setContactModal(true) }>
        {toggle && <img  class="overflow-hidden md:mr-8" onMouseLeave={() => setToggle(!toggle)} src={ContactButtonHoverSVG} {...morph}></img> }
      
        {!toggle && <img  class="mr-8 overflow-hidden" onMouseLeave={() => setToggle(!toggle)} onMouseEnter={() => setToggle(!toggle)} width="120px" src={ContactButtonSVG} {...morph}></img>}
    </div>
  );
}
